...
filters.addEventListener('click', (e) => {
    ...
    const key = (btn.dataset.filter || btn.textContent).trim().toLowerCase()

    coursesList.innerHTML = ''

    const filtered = key === 'all'
        ? courses
        : courses.filter(c => c.subject.toLowerCase() === key || key.includes(String(c.number)))
    ...
    const courseCredits = document.querySelector(".course-credits-number")
